...
global block width                     % for road drawing
global real_landmarks real_car1 real_car2   % true values
global meas_landmark1 meas_landmark2        % measurements
global max_read_distance                    % sensor range
global index_fov index_seen                  % FoV and seen flags
...
measurement_variance = [2;0.2^2];    % Measurement covariance (Var(x) (m) ; Var(y) (radian))
...
%     meas_landmark1(:,l,1) = Measurement(real_car1(:,1), real_landmarks(:,l) + randn(2,1));
% end
% for l = 1:num_landmarks
%     meas_landmark2(:,l,1) = Measurement(real_car2(:,1), real_landmarks(:,l) + randn(2,1));
% end
